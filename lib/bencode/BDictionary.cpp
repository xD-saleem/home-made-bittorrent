 ...
 */
BDictionary::size_type BDictionary::size() const { return itemMap.size(); }
 ...
 */
BDictionary::iterator BDictionary::begin() { return itemMap.begin(); }

/**
 * @brief Returns an iterator to the end of the dictionary.
 */
BDictionary::iterator BDictionary::end() { return itemMap.end(); }
 ...
 */
BDictionary::const_iterator BDictionary::end() const { return itemMap.end(); }
 ...
 * @brief Returns a constant iterator to the end of the dictionary.
 */
BDictionary::const_iterator BDictionary::cend() const { return itemMap.cend(); }
...
std::shared_ptr<BItem> BDictionary::getValue(const std::string& key) {
  for (const auto& item : *this) {
    ...
    if (typeid(*item.second) == typeid(BDictionary)) {
      std::shared_ptr<BDictionary> subDictionary =
          std::dynamic_pointer_cast<BDictionary>(item.second);
    ...
    // Checks if the value which the key maps to is a BList
      ...
      std::shared_ptr<BList> subList =
          std::dynamic_pointer_cast<BList>(item.second);
