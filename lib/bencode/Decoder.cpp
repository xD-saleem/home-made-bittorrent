...
void Decoder::readExpectedChar(std::istream& input, char expected_char) const {
  int c = input.get();
  if (c != expected_char) {
    throw DecodingError(std::string("expected '") + expected_char + "', got '" +
                        static_cast<char>(c) + "'");
  }
}
 ...
 * @par Example
 * @code
 * d3:cow3:moo4:spam4:eggse represents the dictionary {"cow": "moo", "spam":
 * "eggs"} d4:spaml1:a1:bee represents the dictionary {"spam": ["a", "b"]}
 * @endcode
 *
...
std::string::size_type Decoder::readStringLength(std::istream& input) const {
  ...
  bool stringLengthInASCIIReadCorrectly =
      readUpTo(input, stringLengthInASCII, ':');
 ...
 */
std::string Decoder::readStringOfGivenLength(
    std::istream& input, std::string::size_type length) const {
  ...
  if (numOfReadChars != length) {
    throw DecodingError("expected a string containing " +
                        std::to_string(length) + " characters, but read only " +
 ...
 */
void Decoder::validateInputDoesNotContainUndecodedCharacters(
    std::istream& input) {
