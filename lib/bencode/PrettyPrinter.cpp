 ...
 */
void PrettyPrinter::storeCurrentIndent() { prettyRepr += currentIndent; }

/**
 * @brief Increases the current indentation by a single level.
 */
void PrettyPrinter::increaseIndentLevel() { currentIndent += indentLevel; }
...
void PrettyPrinter::decreaseIndentLevel() {
  currentIndent =
      currentIndent.substr(0, currentIndent.size() - indentLevel.size());
}
...
void PrettyPrinter::visit(BString* bString) {
  ...
  prettyRepr +=
      '"' + replace(bString->value(), '"', std::string(R"(\")")) + '"';
}
